...
pub const M5_EVIDENCE_MANIFEST_SCHEMA: &str = "hera.m5_evidence_manifest";
pub const M5_EVIDENCE_MANIFEST_VERSION: u32 = 1;
pub const M5_MAX_EVIDENCE_MANIFEST_BYTES: u64 = 1024 * 1024;
pub const M5_MAX_PUBLIC_ARTIFACT_BYTES: u64 = 4 * 1024 * 1024;
const M5_MAX_EVIDENCE_ARTIFACTS: usize = 512;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5EvidenceManifest {
    pub schema: String,
    pub version: u32,
    pub generated_at: String,
    pub hera_commit: String,
    pub redaction: M5RedactionPolicy,
    pub artifacts: Vec<M5EvidenceArtifact>,
}

impl M5EvidenceManifest {
    pub fn from_path(path: impl AsRef<Path>) -> Result<Self, FixtureLoadError> {
        let path = path.as_ref();
        let raw =
            read_text_file_capped(path, M5_MAX_EVIDENCE_MANIFEST_BYTES, "M5 evidence manifest")?;
        Self::from_json_str(path, &raw)
    }

    pub fn from_json_str(path: impl AsRef<Path>, json: &str) -> Result<Self, FixtureLoadError> {
        let path = path.as_ref().to_path_buf();
        if json.len() as u64 > M5_MAX_EVIDENCE_MANIFEST_BYTES {
            return Err(invalid_schema(
                &path,
                "$",
                format!(
                    "M5 evidence manifest JSON is {} bytes, maximum is {M5_MAX_EVIDENCE_MANIFEST_BYTES}",
                    json.len()
                ),
            ));
        }
    ...
    pub fn validate_public_artifact_files(
        ...
        for (index, artifact) in self.artifacts.iter().enumerate() {
            ...
            let raw =
                read_text_file_capped(&path, M5_MAX_PUBLIC_ARTIFACT_BYTES, "M5 public artifact")?;
    ...
    pub fn artifacts(&self) -> &[M5EvidenceArtifact] {
        &self.artifacts
    }

    fn validate(&self, path: &Path) -> Result<(), FixtureLoadError> {
        if self.schema != M5_EVIDENCE_MANIFEST_SCHEMA {
            return Err(invalid_schema(
                path,
                "schema",
                format!("expected {M5_EVIDENCE_MANIFEST_SCHEMA}"),
            ));
        }
        if self.version != M5_EVIDENCE_MANIFEST_VERSION {
            return Err(invalid_schema(
                path,
                "version",
                format!("expected {M5_EVIDENCE_MANIFEST_VERSION}"),
            ));
        }
        ...
        if self.artifacts.len() > M5_MAX_EVIDENCE_ARTIFACTS {
            return Err(invalid_schema(
                path,
                "artifacts",
                format!(
                    "manifest has {} artifacts, maximum is {M5_MAX_EVIDENCE_ARTIFACTS}",
                    self.artifacts.len()
                ),
            ));
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct M5RedactionPolicy {
    pub version: u32,
    pub updated_at: String,
    pub reject_patterns: Vec<String>,
    pub public_privacy_classes: Vec<M5ArtifactPrivacy>,
}

impl M5RedactionPolicy {
    fn validate(&self, path: &Path) -> Result<(), FixtureLoadError> {
        if self.version == 0 {
            return Err(invalid_schema(
                path,
        ...
        if !self
            ...
            .contains(&M5ArtifactPrivacy::PublicSummary)
            || !self
                .public_privacy_classes
                .contains(&M5ArtifactPrivacy::ScrubbedPublic)
        {
            return Err(invalid_schema(
                path,
...
pub struct M5EvidenceArtifact {
    ...
    pub privacy: M5ArtifactPrivacy,
    pub platform: M5ArtifactPlatform,
    pub reproducibility_status: M5ReproducibilityStatus,
    pub owner: String,
    #[serde(default)]
...
impl M5EvidenceArtifact {
    fn validate(
        &self,
        manifest_path: &Path,
        index: usize,
        redaction: &M5RedactionPolicy,
    ) -> Result<(), FixtureLoadError> {
        ...
        validate_required_string(
            manifest_path,
            format!("artifacts[{index}].owner"),
            &self.owner,
        )?;
        if self.privacy == M5ArtifactPrivacy::RawLocal {
            return Err(invalid_schema(
                manifest_path,
                format!("artifacts[{index}].privacy"),
                "raw_local artifacts must not be listed in the public M5 manifest",
            ));
        }
        ...
        if self.reproducibility_status == M5ReproducibilityStatus::NonReproducible {
            let has_reason = self
    ...
    fn requires_raw_field_scan(&self) -> bool {
        self.path.ends_with(".json")
            || self.path.ends_with(".jsonl")
            || self.artifact_type.contains("replay")
            || self.artifact_type.contains("dogfood")
            || self.artifact_type.contains("session")
            || self.artifact_type.contains("baseline")
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ArtifactPrivacy {
    PublicSummary,
    ScrubbedPublic,
    LocalPrivate,
    RawLocal,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum M5ReproducibilityStatus {
    Reproducible,
    NonReproducible,
    Blocked,
}
...
mod tests {
    ...
    use super::{M5EvidenceManifest, M5ReproducibilityStatus};
    ...
    fn valid_manifest_json(path: &str) -> String {
        format!(
            r#"{{
              "schema": "hera.m5_evidence_manifest",
              "version": 1,
              ...
              "redaction": {{
                ...
                "reject_patterns": ["C:\\\\Users\\\\", "C:/Users/", "/home/", "OPENAI_API_KEY"],
              ...
              "artifacts": [{{
                ...
                "type": "baseline",
                "path": "{path}",
                "source_command": "terminal-cli generate-m5-baseline --output evidence/m5/m5-baseline.json",
                "generated_at": "2026-07-04T00:00:00Z",
    ...
    #[test]
    fn m5_evidence_manifest_validates_checked_in_contract() {
        let root = workspace_root();
        let manifest_path = root.join("evidence/m5/evidence-manifest.json");
        let manifest =
            M5EvidenceManifest::from_path(&manifest_path).expect("M5 manifest should load");

        for expected_id in [
            "m5_baseline",
            "m5_go_no_go_policy",
            "m5_baseline_and_evidence_policy_doc",
            "m5_initial_report",
        ] {
            assert!(
                manifest
                ...
                "missing M5 artifact {expected_id}"
            );
        }
        assert!(
            manifest
                .artifacts()
                .iter()
                .all(|artifact| artifact.owner.trim() != "")
        );
        manifest
            .validate_public_artifact_files(&root)
            .expect("checked-in M5 artifacts should pass redaction validation");
    }

    #[test]
    fn m5_evidence_manifest_rejects_absolute_paths() {
        let json = valid_manifest_json("evidence/m5/m5-baseline.json").replace(
            "evidence/m5/m5-baseline.json",
            "C:/Users/Arthur/private.json",
        );

        let error = M5EvidenceManifest::from_json_str("manifest.json", &json)
            .expect_err("absolute public path must fail");
    ...
    #[test]
    fn m5_evidence_manifest_rejects_raw_local_artifact() {
        let json = valid_manifest_json("evidence/m5/m5-baseline.json").replace(
            "\"privacy\": \"public_summary\"",
            "\"privacy\": \"raw_local\"",
        );
    ...
    #[test]
    fn m5_evidence_manifest_requires_non_reproducible_reason() {
        let json = valid_manifest_json("evidence/m5/m5-baseline.json").replace(
            "\"reproducibility_status\": \"reproducible\"",
        ...
        let error = M5EvidenceManifest::from_json_str("manifest.json", &json)
            .expect_err("non-reproducible artifacts need a reason");

        assert!(matches!(
            error,
            FixtureLoadError::InvalidSchema { field, .. }
                if field == "artifacts[0].non_reproducible_reason"
        ));
    }

    #[test]
    fn m5_evidence_manifest_rejects_private_content_in_public_artifact() {
        let root = temp_dir("hera-m5-evidence-redaction");
        ...
        let manifest = M5EvidenceManifest::from_json_str(
            "manifest.json",
    ...
    #[test]
    fn reproducibility_status_roundtrips() {
        let manifest = M5EvidenceManifest::from_json_str(
            "manifest.json",
            &valid_manifest_json("evidence/m5/m5-baseline.json"),
        )
        .expect("manifest should load");

        assert_eq!(
            manifest.artifacts()[0].reproducibility_status,
            M5ReproducibilityStatus::Reproducible
        );
    }
}
